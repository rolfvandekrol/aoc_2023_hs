...
getNextValueForFactors :: [Rational] -> Rational
getNextValueForFactors factors = sum $ mapWithIndex (\factor i -> factor * (x ^ i)) factors
  where x = toRational $ length factors

solve v = sum $ map (numerator . getNextValueForFactors . augMatrixToFactors . reduceRowEchelonAugMatrix . rowEcholenAugMatrix . getAugMatrix) v
